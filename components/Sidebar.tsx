...
export default function Sidebar({ user }: SidebarProps) {
  ...
  const sidebarContent = (
    ...
    `}>
      ...
      <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
        <p className="px-2 text-xs font-medium text-slate-400 uppercase tracking-wider mb-3">Báo cáo</p>
              ...
              <Link href={item.href} onClick={() => setOpen(false)}
                className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition
                  ${parentActive
                    ...
                    : groupActive ? 'text-blue-600 bg-blue-50'
        ...
        {user.role === 'admin' && (
          <>
            <div className="pt-4 pb-2">
              <p className="px-2 text-xs font-medium text-slate-400 uppercase tracking-wider">Quản trị</p>
            </div>
            {ADMIN_ITEMS.map(item => {
              ...
              return (
                <Link key={item.href} href={item.href} onClick={() => setOpen(false)}
                  className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition
                    ${isActive ? 'bg-blue-600 text-white' : 'text-slate-500 hover:text-slate-900 hover:bg-slate-100'}`}>
                  {item.icon}{item.label}
                </Link>
              )
            })}
      ...
      <div className="px-3 py-4 border-t border-slate-200">
        <div className="flex items-center gap-3 px-3 py-2 mb-2">
          <div className="w-8 h-8 bg-blue-50 rounded-full flex items-center justify-center shrink-0">
            <span className="text-blue-600 text-xs font-bold">{user.full_name.charAt(0).toUpperCase()}</span>
          </div>
        ...
        <button onClick={handleLogout}
          className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-slate-500 hover:text-red-600 hover:bg-red-50 transition">
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8}
  ...
  return (
    <>
      {/* Hamburger button — mobile only (fixed top bar) */}
      <div className="md:hidden fixed top-0 left-0 right-0 z-30 bg-white border-b border-slate-200 px-4 py-3 flex items-center gap-3 shadow-sm">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-slate-800 font-semibold text-sm">Báo Cáo Nội Bộ</span>
        </div>
      </div>
